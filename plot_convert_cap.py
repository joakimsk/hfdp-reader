...
def lowpass_iir(x, fc_hz, dt_s):
    """1st-order low-pass, cutoff fc_hz."""
...
def main():
    ...
    # median first (k=3 or 5)
    p_med = median_filter_1d(p_mbar, k=3)

    # low-pass to e.g. 20 Hz (adjust to taste)
    dt = float(np.median(np.diff(t_s)))  # ~0.0005 for 2 kHz
    p_filt = lowpass_iir(p_med, fc_hz=20.0, dt_s=dt)

    plt.figure()
    plt.plot(t_s, p_filt)
    plt.xlabel("Time (s)")
    plt.ylabel("Pressure (mbar)")
    plt.title("Pressure vs time (median + low-pass)")
    plt.grid(True)
    plt.show()
